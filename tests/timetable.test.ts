...
describe('播种：时段与正课', () => {
  ...
  it('播种 teacher_schedule（含本班与跨班演示行）', () => {
    const db = makeDb();
    const ts = query<Row>(db, 'SELECT * FROM teacher_schedule');
...
describe('纯函数', () => {
  ...
  it('removePeriodSlot 级联删除其下 timetable 与 teacher_schedule 行', () => {
    ...
    const seeded = new Set(query<Row>(db, 'SELECT period_id FROM teacher_schedule').map(r => Number(r.period_id)));
    const zheng = slots.find(s => s.kind === '正课' && !seeded.has(Number(s.id)))!;
    db.prepare('INSERT INTO teacher_schedule (weekday, period_id, class_name, subject, remark) VALUES (?, ?, ?, ?, ?)').run(3, Number(zheng.id), '六年级（2）班', '数学', '');
    const n = (sql: string, id: number) => (db.prepare(sql).get(id) as { n: number }).n;
    expect(n('SELECT COUNT(*) AS n FROM timetable WHERE period_id = ?', Number(zheng.id))).toBe(5);
    expect(n('SELECT COUNT(*) AS n FROM teacher_schedule WHERE period_id = ?', Number(zheng.id))).toBe(1);
