...
import { App, Modal, Button, Input, Select, Space, Popconfirm } from 'antd';
      ...
      <Space direction="vertical" style={{ width: '100%' }} size="small">
        {ordered.map((slot, i) => (
          <Space key={slot.id} style={{ width: '100%' }} align="baseline">
            ...
            <Input size="small" style={{ width: 120 }} defaultValue={String(slot.name)} onBlur={e => void patch(slot.id as number, { name: e.target.value })} />
            <Input size="small" style={{ width: 80 }} defaultValue={String(slot.start_time)} onBlur={e => void patch(slot.id as number, { start_time: e.target.value })} placeholder="HH:mm" />
            <Input size="small" style={{ width: 80 }} defaultValue={String(slot.end_time)} onBlur={e => void patch(slot.id as number, { end_time: e.target.value })} placeholder="HH:mm" />
            <Select size="small" style={{ width: 92 }} value={String(slot.kind)} options={KINDS.map(k => ({ value: k, label: k }))} onChange={v => void patch(slot.id as number, { kind: v })} />
            <Popconfirm title="删除该时段将同时删除其课表与授课行，确定？" onConfirm={async () => { try { await remove(slot.id as number); message.success('已删除'); } catch { message.error('删除失败'); } }}>
